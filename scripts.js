...
// JavaScript to handle dynamic animation
const nav = document.querySelector('nav');
const animation = document.querySelector('.animation');
const links = document.querySelectorAll('nav a');

function moveAnimation(link) {
    const linkCoords = link.getBoundingClientRect();
    const navCoords = nav.getBoundingClientRect();

    animation.style.width = `${link.offsetWidth}px`;
    animation.style.left = `${linkCoords.left - navCoords.left}px`;
}

links.forEach(link => {
    link.addEventListener('mouseover', () => moveAnimation(link));
});

if (links.length > 0) {
    moveAnimation(links[0]); // Default to the first link
}